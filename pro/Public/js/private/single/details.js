...
project.init=function(){
	this.arrs=['',$('#mess').attr('data-arr11'),$('#mess').attr('data-arr12')]
	this.arr1=[$('#mess').attr('data-arr21'),$('#mess').attr('data-arr22'),$('#mess').attr('data-arr23')]
	this.arrs2 =['',['xb','gg'],['cj']]
	this.p1=[1,1];
	this.p1c=[0,0]
	//解析URL并初始化
...
project.initData=function(){
	...
	$.ajax({
		...
		success:function(req){
			...
			// var a = $('#hideP').text();
			$('#article-body').html(req.article.content);
			var txt ='';
			...
			$('#ulis').append(txt);
			$('#uli2').append(text);
			$('.bot1').html(req.useful.useful_num)
			$('.bot2').html(req.useful.useful_total)
		}
	})
}
