...
project.initData = function(){
	...
	$.ajax({
		...
		success:function(req){
			if(req.count==0){
				$('.page-header').append('<p class="page-header-description">“'+decodeURI(url.query)+$('#mess').attr('data-t1')+'</p>')
			}else{
				$('.page-header').append('<p class="page-header-description">“'+decodeURI(url.query)+$('#mess').attr('data-t2')+''+req.count+' '+$('#mess').attr('data-t3')+'</p>');
				...
				for(var i=0;i<req.data.length;i++){
					...
					' </li>'+
					' <li class="meta-data"><time>'+req.data[i].simple_time+$('#mess').attr('data-t5')+'</time></li>'+
					' </ul>'+
