...
project.init=function(){
	this.arrs=['',[$('#mess').attr('data-arr11'),$('#mess').attr('data-arr12'),$('#mess').attr('data-arr13')],[$('#mess').attr('data-arr14'),'']]
	this.arrs2 =['',['xb','gg'],['cj']]
	this.p1=[1,1];
	this.p1c=[0,0]
	//解析URL并初始化
...
project.initData=function(){
	...
	if(url.status==2){
		...
		$.ajax({
			...
			success:function(req){
				...
				$('.see-all-articles').eq(1).html($('#mess').attr('data-t1')+req.count+$('#mess').attr('data-t2'));
	...
	$.ajax({
			...
			success:function(req){
				...
				$('.see-all-articles').eq(0).html($('#mess').attr('data-t1')+req.count+$('#mess').attr('data-t2'));
