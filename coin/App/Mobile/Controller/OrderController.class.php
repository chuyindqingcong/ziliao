...
class OrderController extends CommonController {
	...
	public function index(){
        ...
        //显示委托记录
        $buy_record=$this->getOrdersByType($currency['currency_id'],'buy', 6, 'desc');
        $sell_record=$this->getOrdersByType($currency['currency_id'],'sell', 6, 'asc');
        ...
        //我的成交
        $mine['currency_id']=$currency['currency_id'];
        $mine['status']=2;
        $mine['member_id']=$_SESSION['USER_KEY_ID'];
        $mine_trade = M('Orders')->where($mine)->limit('10')->select();
        $this->assign('mine_trade',$mine_trade);
    ...
    public function getOrders(){
        switch (I('post.type')){
          case 'buy':  $this->ajaxReturn($this->getOrdersByType(I('post.currency_id'),'buy', 6, 'desc'));
          break;
          case 'sell':$this->ajaxReturn($this->getOrdersByType(I('post.currency_id'),'sell', 6, 'asc'));
          break;
        }
    }
    //获取k线
