...
//     	$result = $bitcoin->getinfo();
    ...
    public function chongzhi_function(){
		...
		if($currency['port_number']==23215)
		{
			...
			//$addr="TS3CrYhB1b7KmnaZeCKQiLP4idWwB61z1S";
			$addrlist=array();
			$blocks=$bitcoin->getblockchaininfo();
			//var_dump($blocks);
			$height=$blocks['blocks'];
			//var_dump($height);
			$map['url']=$addr;
			$iary=M('tibi')->where($map)->order('height desc')->find();
			...
			for($i=$a+1;$i<=$height;$i++)
			{
				...
				$tx=$bitcoin->getblock($hash['hash']);
				
				$txarray=$tx['tx'];
				if(!empty($txarray))
				{
					foreach($txarray as $key=>$value)
					{
						$list=$bitcoin->gettxdetail($value);
						if(array_key_exists('desaddr',$list))
						{
							if($list['desaddr']==$addr)
							{
								$addrlist[$key][]=$list;
							}
						}
			...
			else
			{
				...
				foreach($addrlist as $a=>$b)
				{
					...
					foreach($b as $k=>$v)
					{
						...
						$data['url']=$v['desaddr'];//地址
						$data['name']=session('USER_KEY');//标签油箱 
						$data['add_time']=$v['confirmedtime'];//时间
						$data['num']=$v['money']/100000000;//数量
						$data['status']=3;
						$data['check_time']=$v['confirmedtime'];
						$data['height']=$v['confirmHeight'];//写入height
						$data['ti_id']=$v['hash'];//写入height
						$num=M("Currency_user")->where("member_id='{$_SESSION['USER_KEY_ID']}' and currency_id='$id'")->getField('num');
						M("Currency_user")->where("member_id='{$_SESSION['USER_KEY_ID']}' and currency_id='$id'")->setInc("num",$data['num']);//给User表加钱
						$re=M("Tibi")->add($data);
...
//     	$result = $bitcoin->getinfo();
		...
		if($currency['port_number']=23215)
		{
			...
			$id=$bitcoin->sendtoaddresswithfee($from_url,$url,$money,$currency['currency_sell_fee']/100);
	...
	
	{
    	...
    	$currency=M("Currency")->where("currency_id=session('hbid')")->find();//这个是货币
    	if(empty($currency)){
    		return false;
    	}
		...
		if($currency['port_number']==23215)
		{
			...
			//var_dump($height);
			$map['url']=$addr;
			$iary=M('tibi')->where($map)->order('ti_id desc')->find();
			...
			else
			{
				...
				foreach($addrlist as $a=>$b)
				{
					...
					foreach($b as $k=>$v)
					{
						...
						$data['status']=3;
						$data['check_time']=$v['confirmedtime'];
						$data['ti_id']=$v['confirmHeight'];//写入height
