	...
	public function d_pass(){
     ...
     public function j_pass(){
 		if (IS_POST) {
 			...
 			$u_info=M('Member')->where("member_id = {$_SESSION['USER_KEY_ID']}")->find();
 			if ($u_info['pwdtrade']==md5($old_pwd)) {
 				$pwd = I('post.pwd');
	 			$sec_pwd = I('post.sec_pwd');
	 			if ($pwd==$sec_pwd) {
	 				$con['pwdtrade'] = md5($pwd);
	 				$mm=M('Member')->where("member_id = {$_SESSION['USER_KEY_ID']}")->save($con);
	 				if ($mm) {
	 					$this->success('设置交易密码成功！',U('Safe/index'));
	 				}else{
	 					$this->error('设置交易密码失败！');
	 				}
	 			}else{
	 				$this->error('两次输入密码不相等，请检查！');
	 			}
 			}else{
 				$this->error('原密码不对，请检查！');
 			}
 		}else{
 			$this->display();
 		}
     ...
     public function jiaoyi_code(){
		 ...
		 else
		 {
		 	...
		 	$u_info = M('Member')->where("member_id = {$_SESSION['USER_KEY_ID']}")->find();
		 	$this->assign('u_phone',$u_info['phone']);
		 	$this->display();
		 }
        
     }

     public function change_jiaoyi(){
     		if (IS_POST) {
     			$pwd = I('post.pwd');
     			$sec_pwd = I('post.sec_pwd');
     			if ($pwd==$sec_pwd) {
