    ...
    private function guadan($num,$price,$type,$currency){
        ...
        switch ($type){
            case 'buy':
                    ...
                    $fee=($num*$price)/$currency['currency_buy_fee'];//计算手续费
   ...
   //$num 交易数量
    private function trade($currencyId,$type,$num,$price,$diycurrency){
       ...
       //如果没有相匹配的订单，直接返回
        ...
        $mefee=($trade_num*$price)*($diycurrency['currency_'.$type.'_fee']/100);//获取本次订单类型手续费
        ...
        switch ($type){
            ...
            case 'buy':
                ...
                $r[]=$this->setUserMoney($trade_order['member_id'],$trade_order['currency_trade_id'], ($trade_num*$trade_price)-$partyfee, 'inc', 'num');
                ...
                $r[]=$this->addFinance($order['member_id'], 11, '交易手续费',$trade_num*($diycurrency['currency_'.$type.'_fee']/100), 2, $order['currency_id']);
              
                $r[]=$this->addFinance($trade_order['member_id'], 11, '交易手续费',$trade_num*$trade_price*($diycurrency['currency_'.$trade_type.'_fee']/100), 2, $order['currency_trade_id']);
                break;
            case 'sell':
                ...
                $feenumber=$trade_num*($diycurrency['currency_'.$type.'_fee']/100);
                // //执行卖出到账
        ...
        if($order['type']=='buy')
        {
            ...
            //卖出
            $tradeid=$this->addTrade($order['member_id'], $order['currency_id'], $order['currency_trade_id'],$trade_price, $trade_num, $order['type'],$trade_num*($diycurrency['currency_'.$type.'_fee']/100));
            if($tradeid>0)
            {
                $res[]=true;
            }else{
