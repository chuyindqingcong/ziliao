       ...
       if(IS_POST)
       {
            ...
            switch ($currency['port_number']) {
                case '10'://eth
                    ...
                    $i=$actual*1000000000000000000;

                    $value='0x'.base_convert($i, 10, 16);
                    $from=C('ETH_WALLET');
                    $result=$client->personal_transaction($from,$url,$value,"ETHetc2jjw!q2w3e");
                ...
                Vendor('phpRPC.phprpc_client');
                    ...
                    $value='0x'.base_convert($i, 10, 16);
                    $from=C('ETH_WALLET');
                    $result=$client->personal_transaction($from,$url,$value,$currency['qianbao_key']);
                    if(!empty($result['result']))
                ...
                default:
                    ...
                    $bitcoin->walletpassphrase($currency['qianbao_key'],30);
                    $actual=(float)$actual;
                    $hashid=$bitcoin->sendtoaddress($url,$actual);//tongxing $id['hash']
                    $bitcoin->walletlock();
                    if($hashid)
                    {
                        $re['status']=1;
             ...
             else//sheng he bu tong guo
             {
                ...
                $model=new \Think\Model();
                $model->startTrans();
                $result=$model->table("yang_currency_user")->where("member_id='{$user_id}' and currency_id='$currency_id'")->setInc("num",$num);
                $re['status']=4;//tuihui
                $result2=$model->table('yang_tibi')->where("id=".$id)->save($re);
