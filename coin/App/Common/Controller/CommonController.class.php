...
class CommonController extends Controller {
    ...
    public function _initialize(){
        //start seo
        $keywords=M('config')->where(['key'=>'keywords'])->find();
        $this->assign('keywords',$keywords['value']);
        $desc=M('config')->where(['key'=>'desc'])->find();
        $this->assign('description',$desc['value']);
        //end seo
        $user_id = session('USER_KEY_ID');  //判断用户ID
        $this->assign('user_id',$user_id);  //判断用户是否登录
        $u_phone = M('Member')->where("member_id='$user_id'")->find();
        $this->assign('us_name',$u_phone['name']);  //判断用户手机号
        ...
        $link = M('Link');
        $link_info =$link->order('add_time asc')->select();
        //截断友情链接url头，统一写法
        ...
        //客户群
        $k_qun = explode(",",$list['qqqun1']);
        //var_dump($k_qun);
        $this->assign('k_qun',$k_qun);
	...
	protected function  User_status(){
		...
		if($list['status']==0){
			session('STATUS',0);
			$this->redirect('Safe/shenfen','',1,"<script>alert('请填写个人信息')</script>");
			exit();
		}
	}
    ...
    public function getCurrencyMessageById($id){
        ...
        $conditon['currency_id']=$id;
        $re=M('Trade')->where($conditon)->order('price')->find();
       // $re=M('Trade')->where($where)->where("add_time<{$tg}")->order('price')->find();
        //p($re);
    ...
    //获取全部币种信息
    public function currency_10(){
        $list=M('Currency')->where('is_line=1 ')->order('sort')->limit('10')->select();
        return $list;
    }  
    //获取全部币种信息
    public function currency_15(){
        $list=M('Currency')->where('is_line=1 ')->order('sort')->limit('13')->select();
        return $list;
    }  
    //获取单独币种信息
    ...
    *  /**
    ...
    //修正众筹表 计算剩余数量  修改状态
     ...
     protected  function  getCurrencyByCurrencyId($currency_id=0){
		if (empty($currency_id)) {
     	...
     	//获取交易币种信息
     	$list= M('Currency')->field("currency_id,currency_mark,currency_name,currency_logo,price_up,price_down,currency_buy_fee,currency_sell_fee,trade_currency_id,is_lock,rpc_url,rpc_pwd,rpc_user,port_number,currency_all_tibi,qianbao_key")->where($where)->select();
    	if (!empty($currency_id)) {
    		return $list[0];  	
     	}else{
     		return $list;
    ...
    //获取全部币种信息
    public function currency_4(){
        $list=M('Currency')->where('is_line=1 ')->order('sort')->limit('6')->select();
        return $list;
    }     
     /**
      * 设置用户资金表 字段值
    ...
    protected function getOrdersByStatus($status,$num,$currency_id){
        $where['currency_id']=$currency_id;
        $where['status']=$status;
        $orders = M('Orders')->where($where)->limit($num)->order('trade_time desc')->select();
      ...
      */
       protected function getOrdersByType($currencyid,$type,$num,$order){
         ...
         foreach ($list as $k=>$v){
             ...
             // $list[$k]['je']=number_format(($v['num']-$v['trade_num'])*$v['price'],2,'.','');
             // $list[$k]['wtlnum']=number_format(($v['num']-$v['trade_num']),4,'.','');

             $a=($v['num']-$v['trade_num'])*$v['price'];
             $b=explode('.',$a);
             if (sizeof ( $b ) > 1) 
             {
                $decimal = end ( $b );
	...
	public function ajaxSendEmail()
     {
         ...
         $body="尊敬的九交网用户：</br>
欢迎您登录九交网，您的验证码是:".$yzm."
</br>如非本人操作，请尽快联系客服冻结账户以确保账户安全。
    </br>客服热线：4008-798-786
    </br>客服服务时间：周一至周日：8：00-24：00";

         

         $re=sendMail($this->config['EMAIL_HOST'],$this->config['EMAIL_USERNAME'],$this->config['EMAIL_PASSWORD'],$email, "第九交易网", $body);

         if($re)
         {
            $data['stauts']=1;
            $data['info'] = "邮件发送成功，请注意查收";
            session('yzm',$yzm);
            $this->ajaxReturn($data);
            exit;
         }
