...
class TongjiController extends AdminController {
     ...
     public function fee()
   {
      ...
      if(I('get.start')==true && I('get.endtime')==true)
      {
        ...
        $invitwhere['ctime'] = array('BETWEEN',array($start,$endtime));
        $withdrawwhere['add_time']=array('BETWEEN',array($start,$endtime));
      }else if(I('get.start')==true)
      {
        $start=strtotime(I('get.start'));
        $invitwhere['ctime']=array('EGT',$start);
        $withdrawwhere['add_time']=array('EGT',$start);
      }else if(I('get.endtime')==true)
      {
        $endtime=I('get.endtime');
        $invitwhere['ctime']=array('ELT',$endtime);
        $withdrawwhere['add_time']=array('ELT',$endtime);
      }
      $invit=M('invit');
      $cfee=array();
      $invitwhere['rebatetype']=1;//获取奖励的总币数
      $cfee['invit_coin']=$invit->where($invitwhere)->sum('cfee');
      $invitwhere['rebatetype']=2;//获取奖励的RMB金额
      $cfee['invit_rmb']=$invit->where($invitwhere)->sum('cfee');
      $withdrawwhere['status']=2;
      $withdraw=M('withdraw');
      $cfee['withdraw']=$withdraw->where($withdrawwhere)->sum('withdraw_fee');
