...
class PendingController extends AdminController {
	...
	public function index() {
		...
		$status = I('get.status', '');
		$start=strtotime(I('get.start'));//获取开始时间并转换为时间戳格式
		$endtime=strtotime(I('get.endtime'));//获取截至时间并转换为时间戳格式
		$where = array();
		if (!empty($cardname))
		...
		if($start==true && $endtime==true)
		{
			//存在开始和截止时间
			$where['w.add_time']=array('between',array($start,$endtime));
		}else if($start==true)
		{
			$where['w.add_time']=array('EGT',$start);
		}else if($endtime==true)
		{
			$where['w.add_time']=array('ELT',$endtime);
		}
		$count = $withdraw
