...
class PayController extends AdminController {
    ...
    public function payByMan(){
            ...
            if(!empty($member_id)){
                $where[C("DB_PREFIX")."pay.member_id"]=$member_id;
            }
            if(I('add_time') == true && I('end_time') == true)
            {
                 $where[C("DB_PREFIX")."pay.add_time"]=array('BETWEEN',array(strtotime(I('add_time')),strtotime(I('end_time'))));
            }else if(I('add_time') == true)
            {
                $where[C("DB_PREFIX")."pay.add_time"]=array('GT',strtotime(I('add_time')));
            }else if(I('end_time') == true)
            {
                $where[C("DB_PREFIX")."pay.add_time"]=array('ELT',strtotime(I('end_time')));
            }
            if($status==4){
                $where[C("DB_PREFIX")."pay.status"]=0;
            }
            if($status==false)
            {
                unset($where[C("DB_PREFIX")."pay.status"]);
            }
     ...
     public function payUpdate(){
     	...
     	if($_POST['status']==1){
            ...
            $rs=M('Member')->where("member_id='".$list['member_id']."'")->setInc('cmoney',$list['count']);
     		//添加财务日志
     ...
     public function admRecharge(){
     	if(IS_POST){
     		...
     		if($data['currency_id']==0){
                ...
                $r[] = M('Member')->where(array('member_id'=>$data['member_id']))->setInc('cmoney',$data['money']);
     		}else{
