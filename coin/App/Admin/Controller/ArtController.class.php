...
class ArtController extends AdminController{
    ...
    public function insert(){
        ...
        if(IS_POST){
            ...
            $data['content'] =  htmlspecialchars(I('post.content','','html_entity_decode'));//内容
            $data['position_id'] = I('post.category_id');//类型  根据类型表的id 对比
            $data['add_time'] = time();//添加时间
            $data['sign'] =I('sign');//是否标红
            $data['zhiding'] =I('zhiding');//是否置顶
            $data['toutiao'] =I('toutiao');//是否头条
            $data['seo'] =I('seo');//seo优化信息
            $data['atter'] =I('atter');//简介
			$data['showdate'] =strtotime(I('post.showdate'));
			$pic=$_FILES['picture'];
    ...
    private function getContentArticleByWhere($where,$Page){
    	 $info = M('Article')->join(C('DB_PREFIX')."article_category ON ".C('DB_PREFIX')."article.position_id = ".C('DB_PREFIX')."article_category.id")
				    ->where($where)
				    ->order('article_id desc,sort')
				    ->limit($Page->firstRow.','.$Page->listRows)
				    ->select();
    	return $info;
    }
	
	private function uploadpic($image) { 
