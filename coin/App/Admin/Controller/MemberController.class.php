...
class MemberController extends AdminController {
    ...
    public function saveMember(){
        ...
        if(IS_POST){
            ...
            if($date=I('post.invit')==true)
            {
                $_POST['invit_time']=strtotime(I('post.invit'));
            }
    ...
    public function show(){
    	...
    	$info = $currency->join(C("DB_PREFIX")."currency ON ".C("DB_PREFIX")."currency_user.currency_id = ".C("DB_PREFIX")."currency.currency_id")
    			...
    			->select();
    	$member_info = $member->field('member_id,name,phone,email,pic1,pic2,pic3')->where($where)->find();
    ...
    public function shenfenrz()
    {
        if(IS_POST)
        {
            ...
            $data['shenhestatus']=$status;
            $re=M('member')->where('member_id='.$memberid)->save($data);
            echo $data['shenhestatus'];
            echo $memberid;
    ...
    public function delsf()
    {
        if(IS_POST)
        {
            $memberid=I('post.memberid');

            $result=M('member')->where('member_id='.$memberid)->find();
            // if($result['pic1'])
            // {

            //     unlink(__APP__.$result['pic1']);
            // }

            // if($result['pic2'])
            // {

            //     unlink(__APP__.$result['pic2']);
            // }
            // 
            echo __APP__.$result['pic2'];
